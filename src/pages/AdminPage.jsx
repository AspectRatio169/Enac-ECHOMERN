...
import {
  ...
  createReward,
  deleteReward,
  addCouponCodesToReward,
  getCouponCodesForReward,
  ...
  return (
    <div>
      <label className="font-display font-medium text-xs text-bark/60 mb-1.5 block">
        Brand Logo{" "}
        <span className="font-body font-normal text-bark/35">
...
function VerifyDialog({ submission, onApprove, onReject, onClose, isOwn }) {
  ...
  const codeMatches =
    inputCode.trim().toUpperCase() === storedCode.toUpperCase();
  const items = (() => {
    try {
      return JSON.parse(submission.items);
    } catch {
      return [];
    }
  })();
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-display font-bold text-lg text-moss">
              Verify Deposit
            </h2>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-eco-100 transition-colors"
          >
        ...
        <div className="bg-eco-50 rounded-2xl p-4 mb-5">
          <div className="flex flex-wrap gap-1 mb-1">
            {items.map((item, i) => (
              <span
                key={i}
                className="font-body text-xs bg-white text-bark/70 px-2 py-0.5 rounded-full border border-eco-100"
              >
        ...
        {isOwn ? (
          <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-100 text-yellow-700 rounded-2xl p-3 mb-5">
            ...
            <span className="font-body text-xs">
              This is your own submission. You cannot approve it.
            </span>
          </div>
        ) : (
          <>
            <p className="font-body text-sm text-bark/60 mb-3">
              Enter the code written on the physical bag:
            </p>
            <input
              type="text"
              value={inputCode}
            ...
            {inputCode !== "" && (
              <p
                className={`font-mono text-xs mb-4 ${codeMatches ? "text-eco-600" : "text-red-500"}`}
              >
                {codeMatches
                  ? "✓ Code matches — you can approve"
                  : "✗ Code does not match"}
              </p>
            )}
          </>
        ...
        {error && (
          <div className="bg-red-50 border border-red-100 text-red-700 rounded-xl p-3 mb-4 font-body text-xs">
            {error}
          </div>
        )}
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={loading || !codeMatches || isOwn}
...
function AddCodesDialog({ reward, onSave, onClose }) {
  ...
  function handleCsvFile(e) {
    ...
    reader.onload = (ev) => {
      ...
      const cells = text
        ...
        .filter((c) => c !== "");
      const codes =
        /\d/.test(cells[0]) || cells[0] === cells[0].toUpperCase()
          ? cells
          : cells.slice(1);
      setRaw(codes.join(", "));
    };
  ...
  async function handleSave() {
    ...
    setError("");
    try {
      await onSave(reward._id, parsed);
      onClose();
    } catch (e) {
      setError(e.message);
    } finally {
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8">
        ...
        {parsed.length > 0 && (
          <div className="bg-eco-50 border border-eco-100 rounded-xl p-3 mb-4">
            <p className="font-mono text-xs text-bark/50 mb-1">
              {parsed.length} code{parsed.length !== 1 ? "s" : ""} detected:
            </p>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 bg-eco-50 text-moss font-display font-semibold text-sm py-3 rounded-2xl hover:bg-eco-100 transition-colors"
          >
            Cancel
          </button>
          <button
          ...
          >
            {loading
              ? "Saving…"
              : `Add ${parsed.length > 0 ? parsed.length : ""} Code${parsed.length !== 1 ? "s" : ""}`}
          </button>
        </div>
      </div>
    </div>
...
function CodesPanel({ reward, onCodesChanged }) {
  ...
  async function fetchCodes() {
    setLoading(true);
    try {
      const res = await getCouponCodesForReward(reward._id);
      setCodes(res);
    } catch {
  ...
  async function handleDelete(codeDoc) {
    ...
    setDeletingId(codeDoc._id);
    try {
      await deleteCouponCode(codeDoc._id);
      setCodes((prev) => prev.filter((c) => c._id !== codeDoc._id));
      onCodesChanged();
    } catch {
      /* deletion failed — UI unchanged */
    } finally {
      setDeletingId(null);
    }
  }
  ...
  if (loading)
    return (
      <div className="mt-3 space-y-2">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-8 bg-eco-50 rounded-xl animate-pulse" />
        ))}
      </div>
    );
  if (codes.length === 0)
  ...
  return (
    <div className="mt-3 space-y-1.5">
      <div className="flex gap-3 mb-3">
        <span className="font-mono text-xs bg-eco-100 text-eco-700 px-3 py-1 rounded-full">
          {unused.length} available
        </span>
        <span className="font-mono text-xs bg-bark/10 text-bark/50 px-3 py-1 rounded-full">
          {used.length} used
        </span>
      </div>
      {unused.map((c) => (
        <div
          key={c._id}
          className="flex items-center justify-between bg-eco-50 border border-eco-100 rounded-xl px-3 py-2 gap-2"
        >
          <span className="font-mono text-xs text-moss font-bold tracking-wide">
            {c.code}
          </span>
          <button
      ...
      {used.map((c) => (
        <div
          key={c._id}
          className="flex items-center justify-between bg-bark/5 border border-bark/10 rounded-xl px-3 py-2 gap-2 opacity-50"
        >
          <span className="font-mono text-xs text-bark/50 line-through tracking-wide">
            {c.code}
          </span>
...
function AddRewardForm({ onCreated, onCancel }) {
  ...
  const quantity = isMultiUse
    ? Number(form.multiUseMaxCount) || 0
    : parsedCodes.length;
  ...
  async function handleSubmit(e) {
    ...
    if (isMultiUse) {
      if (!form.multiUseCode.trim()) {
        setError("Shared coupon code is required.");
        return;
      }
      if (!form.multiUseMaxCount || quantity < 1) {
        setError("Enter a valid max redemption count.");
        return;
      }
    } else {
      if (quantity === 0) {
        setError("Paste at least one coupon code.");
        return;
      }
    }
    setSaving(true);
    ...
    try {
      ...
      if (!isMultiUse) await addCouponCodesToReward(newReward._id, parsedCodes);
      onCreated();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const textFields = [
    {
      key: "title",
      label: "Voucher Name",
      placeholder: "10% off on Starbucks",
      span: 2,
    },
    {
      key: "brandName",
    ...
    },
    {
      key: "partner",
      label: "Partner",
      placeholder: "Starbucks India",
      span: 1,
    },
    {
      key: "description",
      label: "Description",
      placeholder: "Get 10% off (up to ₹100) on your next Starbucks order",
      span: 2,
    },
  ];
  ...
  return (
    <div className="bg-white rounded-3xl border border-eco-100 p-7">
      <div className="flex items-center gap-3 mb-7">
        ...
        <div>
          <h3 className="font-display font-bold text-lg text-moss">
            New Reward
          </h3>
          <p className="font-body text-xs text-bark/45 mt-0.5">
            Fill in all details and paste the coupon codes from the brand.
          </p>
        </div>
      </div>
      {error && (
              ...
              {[
                {
                  ...
                  desc: "Each user gets a unique code",
                },
                {
                  value: "multi_use",
                  label: "Shared Code",
                  desc: "One code, used up to N times",
                },
              ].map((opt) => (
                <button
                  key={opt.value}
            ...
            <div className="sm:col-span-2">
              <div className="flex items-center justify-between mb-1.5">
                <label className="font-display font-medium text-xs text-bark/60">
                  Coupon Codes
                  <span className="font-body font-normal text-bark/35 ml-1.5">
                    comma or newline separated
                  </span>
                </label>
                <input
              ...
              <textarea
                ...
                onChange={(e) => {
                  set("couponCodesRaw", e.target.value);
                  setCsvFile(null);
                }}
              ...
              {quantity > 0 && (
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {parsedCodes.slice(0, 6).map((c, i) => (
                    <span
                      key={i}
                      className="font-mono text-xs bg-eco-100 text-eco-700 px-2.5 py-0.5 rounded-lg"
                    >
                      {c}
                    </span>
                  ))}
                  {quantity > 6 && (
                    <span className="font-mono text-xs bg-bark/10 text-bark/45 px-2.5 py-0.5 rounded-lg">
                      +{quantity - 6} more
                    </span>
                  )}
                </div>
...
export default function AdminPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      setRewards(r);
      if (r.length > 0) {
        const counts = await getAvailableCodeCounts(r.map((rw) => rw._id));
  ...
  async function refreshCodeCounts() {
    ...
    const counts = await getAvailableCodeCounts(rewards.map((r) => r._id));
    setCodeCounts(counts);
  }

  function isOwnSubmission(sub) {
    if (sub.userId === user._id) return true;
    const submitter = users.find(
      (u) => u._id === sub.userId || u.userId === sub.userId,
    );
    return !!(submitter && profile?.email && submitter.email === profile.email);
  }

  async function handleApprove(sub) {
    await updateSubmissionStatus(sub._id, "verified");
    setSubmissions((prev) =>
      prev.map((s) => (s._id === sub._id ? { ...s, status: "verified" } : s)),
    );
    setVerifyingSub(null);
  }
  async function handleReject(sub) {
    await updateSubmissionStatus(sub._id, "rejected");
    setSubmissions((prev) =>
      prev.map((s) => (s._id === sub._id ? { ...s, status: "rejected" } : s)),
    );
    setVerifyingSub(null);
  }
  ...
  async function handleToggleAdmin(u) {
    ...
    if (
      !confirm(
        `${isAdmin ? "Remove admin from" : "Make admin"} ${u.name || u.email}?`,
      )
    )
      return;
    setRoleActionId(u._id);
    try {
      if (isAdmin) {
        await demoteToUser(u._id);
        setUsers((prev) =>
          prev.map((x) => (x._id === u._id ? { ...x, role: "user" } : x)),
        );
      } else {
  ...
  async function handleToggleReward(reward) {
    try {
      await updateReward(reward._id, { available: !reward.available });
      setRewards((prev) =>
        prev.map((r) =>
          r._id === reward._id ? { ...r, available: !r.available } : r,
        ),
      );
    } catch (e) {
      setError(e.message);
  ...
  async function handleDeleteReward(reward) {
    if (
      !confirm(
        `Delete "${reward.title}"? All coupon codes will also be deleted.`,
      )
    )
      return;
    try {
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="h-32 bg-white rounded-3xl border border-eco-100 animate-pulse"
              />
            ))}
          </div>
        ) : (
            ...
            {tab === "overview" && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {[
                  ...
                  <div
                    key={label}
                    className="bg-white rounded-3xl p-7 border border-eco-100 shadow-sm"
                  >
                    <div
                      className={`w-11 h-11 ${bg} rounded-2xl flex items-center justify-center mb-4`}
                    >
                      <Icon className={`w-5 h-5 ${color}`} strokeWidth={1.5} />
                    </div>
                    <div className="font-display font-bold text-3xl text-moss">
                      {value}
                    </div>
                    <div className="font-body text-bark/55 text-sm mt-1">
                      {label}
                    </div>
                  </div>
                ))}
            ...
            {tab === "submissions" && (
              <div className="bg-white rounded-3xl border border-eco-100 p-7">
                <h2 className="font-display font-semibold text-moss mb-6">
                  All Submissions
                  <span className="font-mono text-xs text-bark/40 ml-3">
                    {submissions.length} total
                  </span>
                </h2>
                {submissions.length === 0 ? (
                  <p className="font-body text-bark/45 text-sm text-center py-10">
                    No submissions yet.
                  </p>
                ) : (
                  <div className="space-y-0">
                    {submissions.map((sub) => {
                      ...
                      return (
                        <div
                          key={sub._id}
                          className="py-4 border-b border-eco-50 last:border-0"
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                            <div className="min-w-0">
                              <div className="flex flex-wrap gap-1 mb-1">
                                {items.map((item, i) => (
                                  <span
                                    key={i}
                                    className="font-body text-xs bg-eco-50 text-bark/70 px-2 py-0.5 rounded-full"
                                  >
                                    {item.itemType} ×{item.quantity}
                                  </span>
                                ))}
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <span
                                className={`font-mono text-xs px-2.5 py-1 rounded-full ${statusStyle[sub.status] || "bg-eco-50 text-bark/50"}`}
                              >
                                {sub.status}
                              </span>
                              {isPending && (
              ...
              <div className="bg-white rounded-3xl border border-eco-100 p-7">
                ...
                {users.length === 0 ? (
                  ...
                  <div className="space-y-0">
                    {users.map((u) => {
                      const isSelf =
                        u._id === profile?._id || u.email === profile?.email;
                      ...
                      return (
                        <div
                          key={u._id}
                          className="flex flex-col sm:flex-row sm:items-center sm:justify-between py-4 border-b border-eco-50 last:border-0 gap-2"
                        >
                          <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {isSuperAdmin && (
                              <span className="font-mono text-xs bg-moss text-cream px-2.5 py-1 rounded-full">
                                superadmin
                              </span>
                            )}
                            {isAdmin && (
                              <span className="font-mono text-xs bg-moss/10 text-moss px-2.5 py-1 rounded-full">
                                admin
                              </span>
                            )}
                            {u.isVerified ? (
                              <span className="font-mono text-xs bg-eco-100 text-eco-700 px-2.5 py-1 rounded-full">
                                verified
                              </span>
                            ) : (
                              <span className="font-mono text-xs bg-yellow-50 text-yellow-700 px-2.5 py-1 rounded-full">
                                unverified
                              </span>
                            )}
                            {canToggle && (
                  ...
                  {rewards.length === 0 ? (
                    <div className="text-center py-10">
                      ...
                      <p className="font-body text-bark/35 text-xs mt-1">
                        Click "New Reward" above to add one
                      </p>
                    </div>
                  ) : (
                    <div className="space-y-0">
                      {rewards.map((r) => {
                        ...
                        return (
                          <div
                            key={r._id}
                            className="py-5 border-b border-eco-50 last:border-0"
                          >
                            <div className="flex items-start gap-3">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="flex items-center gap-2 mt-2 flex-wrap">
                                  <span className="font-mono text-xs text-bark/40">
                                    {r.brandName}
                                  </span>
                                  <span className="font-mono text-xs text-bark/25">
                                    ·
                                  </span>
                                  ...
                                  <span
                                    className={`font-mono text-xs flex items-center gap-1 ${stockEmpty ? "text-red-500" : "text-eco-600"}`}
                                  >
                                  ...
                                  {isMultiUse && (
                                    <span className="font-mono text-xs bg-moss/10 text-moss px-2 py-0.5 rounded-full">
                                      multi-use
                                    </span>
                                  )}
                                </div>
                            ...
                            {!isMultiUse && isExpanded && (
                              <CodesPanel
                                reward={r}
                                onCodesChanged={refreshCodeCounts}
                              />
                            )}
                          </div>
                        );
