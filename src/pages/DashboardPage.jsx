  ...
  return (
    <div className="fixed inset-0 bg-bark/40 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center">
        <div className="w-16 h-16 bg-eco-100 rounded-2xl flex items-center justify-center mx-auto mb-5">
          <CheckCircle className="w-8 h-8 text-moss" strokeWidth={1.5} />
        </div>
...
function CouponCode({ code }) {
  ...
  return (
    <button
      ...
      title="Click to copy"
    >
      <span className="font-mono text-xs font-bold text-moss tracking-widest">
        {code}
      </span>
      {copied ? (
        <CheckCircle className="w-3 h-3 text-eco-500 shrink-0" />
      ) : (
        <ClipboardCopy className="w-3 h-3 text-bark/30 group-hover:text-moss shrink-0 transition-colors" />
      )}
    </button>
  );
}
...
function SubmitModal({ onClose, onSuccess, userId, groupId }) {
  ...
  const totalPoints = items.reduce((sum, item) => {
    return (
      sum +
      (item.itemType ? (ITEM_POINTS[item.itemType] || 10) * item.quantity : 0)
    );
  }, 0);
  function addItem() {
  ...
  async function handleSubmit(e) {
    ...
    if (!binId) {
      setError("Select a collection bin.");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className="fixed inset-0 bg-bark/40 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 relative max-h-[90vh] overflow-y-auto">
        <button
          onClick={onClose}
          className="absolute top-5 right-5 w-8 h-8 flex items-center justify-center rounded-full hover:bg-eco-100 transition-colors"
        >
          <X className="w-4 h-4 text-bark/50" />
        </button>
        <h2 className="font-display font-bold text-xl text-moss mb-1">
          Log E-Waste Deposit
        </h2>
        <p className="font-body text-bark/50 text-sm mb-6">
          Add all items in this bag.
        </p>
        {error && (
          <div className="bg-red-50 border border-red-100 text-red-700 rounded-2xl p-3 mb-4 font-body text-sm">
            {error}
          </div>
        )}
        <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {items.map((item, index) => (
                <div key={index} className="flex gap-2 items-center">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <span className="font-display font-bold text-sm text-moss w-6 text-center">
                      {item.quantity}
                    </span>
                    <button
                      type="button"
          ...
          {totalPoints > 0 && (
            <div className="bg-eco-50 border border-eco-100 rounded-2xl p-3 flex items-center justify-between">
              <span className="font-body text-sm text-bark/60">
                Total points for this bag
              </span>
              <span className="font-mono font-bold text-eco-600">
                +{totalPoints} pts
              </span>
            </div>
          )}
          {groupId && (
            <div className="bg-moss/10 border border-moss/20 rounded-2xl p-3 flex items-center gap-2">
              ...
              <span className="font-body text-sm text-moss">
                Points will also be added to your group
              </span>
            </div>
          )}
          <div>
            <label className="font-display font-medium text-sm text-bark/70 mb-3 block">
              Collection Bin
            </label>
...
export default function DashboardPage() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      if (rwds.length > 0) {
        const counts = await getAvailableCodeCounts(rwds.map((r) => r._id));
  ...
  async function handleDelete(submission) {
    ...
    try {
      await deleteSubmission(submission._id); // ✅ no extra args
  ...
  async function handleRedeem(reward) {
    ...
    try {
      await redeemReward(user._id, reward._id);
      await refreshProfile();
      await loadAll();
    } catch (e) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-10 gap-4 flex-wrap">
          <div>
            ...
            <p className="font-body text-bark/55 mt-2 text-sm">
              Track your contributions and redeem rewards.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
          {[
            ...
            <div
              key={label}
              className="bg-white rounded-3xl p-7 border border-eco-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300"
            >
              <div
            ...
            {loadingData ? (
              ...
              <div className="text-center py-10">
                ...
                <p className="font-body text-bark/35 text-xs mt-1">
                  Log your first e-waste deposit above
                </p>
              </div>
            ) : (
              <div className="space-y-0">
                {submissions.slice(0, 8).map((s) => {
                  ...
                  return (
                    <div
                      key={s._id}
                      className="py-3.5 border-b border-eco-50 last:border-0"
                    >
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            ...
            >
              ...
              <p className="font-body text-bark/50 text-xs leading-relaxed">
                {profile?.groupId
                  ? "View your group stats and leaderboard."
        ...
        <div className="bg-white rounded-3xl border border-eco-100 p-7 mb-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="font-mono text-xs text-bark/40 ml-auto">
          ...
          {loadingData ? (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-20 bg-eco-50 rounded-2xl animate-pulse"
                />
              ))}
            </div>
          ) : rewards.length === 0 ? (
            ...
            <div className="space-y-0">
              {rewards.map((r) => {
                const codesLeft = codeCounts[r._id] ?? 0;
                ...
                const timesRedeemed = redemptions.filter(
                  (rd) => rd.rewardId === r._id,
                ).length;
                return (
                  <div
                    key={r._id}
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="font-body text-xs text-bark/55 mt-0.5 leading-relaxed">
                          {r.description}
                        </p>
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                          <span
                            className={`font-mono text-xs flex items-center gap-1 ${outOfStock ? "text-red-400" : "text-bark/40"}`}
                          >
        ...
        <div className="bg-white rounded-3xl border border-eco-100 p-7">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-9 h-9 bg-eco-100 rounded-xl flex items-center justify-center">
              <Ticket className="w-4 h-4 text-moss" strokeWidth={1.5} />
            </div>
          ...
          {loadingData ? (
            <div className="space-y-3">
              {[1, 2].map((i) => (
                <div
                  key={i}
                  className="h-16 bg-eco-50 rounded-2xl animate-pulse"
                />
              ))}
            </div>
            ...
            <div className="text-center py-10">
              <div className="w-14 h-14 bg-eco-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <Ticket className="w-7 h-7 text-eco-300" strokeWidth={1.5} />
              </div>
              <p className="font-display font-medium text-bark/50 text-sm">
                No coupons yet
              </p>
              <p className="font-body text-bark/35 text-xs mt-1">
                Redeem a reward above to get your first coupon
              </p>
            </div>
          ) : (
            <div className="space-y-0">
              {redemptions.map((redeem) => {
                ...
                return (
                  <div
                    key={redeem._id}
                    className="flex items-center justify-between py-4 border-b border-eco-50 last:border-0 gap-3 flex-wrap"
                  >
