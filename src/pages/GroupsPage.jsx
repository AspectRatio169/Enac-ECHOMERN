...
export default function GroupsPage() {
  ...
  async function loadAll(overrideGroupId) {
    ...
    try {
      const gId =
        overrideGroupId !== undefined ? overrideGroupId : currentGroupId;
  ...
  async function handleInvite(e) {
    ...
    if (memberCount >= MAX_MEMBERS) {
      setError(
        `Groups are limited to ${MAX_MEMBERS} members to keep things fair. This group is full.`,
      );
      return;
    }
      ...
      <div className="max-w-5xl mx-auto">
        <div className="mb-10">
          ...
          <p className="font-body text-bark/55 mt-2 text-sm">
            Team up and earn bonus eco-points together. Max {MAX_MEMBERS}{" "}
            members per group.
          </p>
        </div>
        ...
        <div className="bg-eco-50 border border-eco-100 rounded-2xl p-4 mb-6 flex items-start gap-3">
          ...
          <div className="font-body text-xs text-bark/70 leading-relaxed">
            <span className="font-semibold text-moss">Group Bonus:</span> For
            every{" "}
            <span className="font-semibold">
              {BONUS_INTERVAL} group credits
            </span>{" "}
            your group earns, each qualifying member receives{" "}
            <span className="font-semibold">{BONUS_PER_MEMBER} eco-point</span>{" "}
            added to their personal balance (useable for rewards).
            <br />
            <span className="text-bark/50 mt-1 block">
              Qualifying = you must have an active submission under this group
              made within the last 45 days. Group credits shown on the
              leaderboard are <em>not</em> spendable — only your personal
              eco-points are.
            </span>
          </div>
        </div>
                    ...
                    {activeTab === "group" && (
                      <div className="p-7">
                        <div className="flex items-start justify-between mb-6">
                          <div>
                            ...
                            <p className="font-body text-bark/50 text-sm">
                              ...
                              {groupFull && (
                                <span className="ml-2 font-mono text-xs bg-yellow-50 text-yellow-700 px-2 py-0.5 rounded-full border border-yellow-100">
                                  Full
                                </span>
                              )}
                            </p>
                          ...
                          <div className="text-right">
                            ...
                            {bonusMilestones > 0 && (
                              <div className="font-mono text-xs text-moss mt-1">
                                {bonusMilestones} bonus
                                {bonusMilestones !== 1 ? "es" : ""} recieved
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        {(() => {
                          ...
                          const nextMilestone =
                            (Math.floor(pts / BONUS_INTERVAL) + 1) *
                            BONUS_INTERVAL;
                          const progressPct =
                            ((pts % BONUS_INTERVAL) / BONUS_INTERVAL) * 100;
                          return (
                            <div className="bg-eco-50 border border-eco-100 rounded-2xl p-4 mb-6">
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="font-mono text-xs font-bold text-moss">
                                  {pts % BONUS_INTERVAL}/{BONUS_INTERVAL}{" "}
                                  credits
                                </span>
                              </div>
                              ...
                              <p className="font-body text-xs text-bark/45 mt-2">
                                {nextMilestone - pts} more group credits → each
                                qualifying member earns +{BONUS_PER_MEMBER}{" "}
                                eco-point
                              </p>
                            </div>
                          );
                        ...
                        <div className="border-t border-eco-100 pt-6">
                          ...
                          {groupFull ? (
                            <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-100 text-yellow-700 rounded-2xl p-3 font-body text-sm">
                              <AlertTriangle className="w-4 h-4 shrink-0" />
                              This group has reached the maximum of{" "}
                              {MAX_MEMBERS} members. No more invites can be
                              sent.
                            </div>
                          ) : (
                            <form
                    ...
                    {activeTab === "achievements" && (
                      <div className="p-7">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <h3 className="font-display font-semibold text-moss">
                            Group Achievements
                          </h3>
                        </div>
                        <p className="font-body text-xs text-bark/50 mb-6">
                          Eco-points earned by members via group bonuses. Each
                          milestone (+{BONUS_INTERVAL} group credits) awards
                          qualifying members +{BONUS_PER_MEMBER} personal
                          eco-point.
                        </p>
                        ...
                        {achievements.filter(
                          ...
                          <div className="text-center py-10">
                            ...
                            <p className="font-body text-bark/35 text-xs mt-1">
                              Earn {BONUS_INTERVAL} group credits together to
                                    ...
                                    <div className="min-w-0">
                                      <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <p className="font-display font-semibold text-sm text-moss">
                                          First {ach.milestone * BONUS_INTERVAL}{" "}
                                          group credits!
                                        </p>
                                      </div>
                                      <p className="font-body text-xs text-bark/50 ml-8">
                                        Group reached{" "}
                                        {ach.milestone * BONUS_INTERVAL} credits
                                        ·{" "}
                                        {new Date(
                                          ach.awardedAt,
                                          ...
                                          <div className="mt-2 ml-8 space-y-1">
                                            {ach.recipients.map((r, i) => (
                                              ...
                                              >
                                                ...
                                                {r.awarded ? (
                                                  <span className="font-mono text-xs text-eco-600">
                                                    +{BONUS_PER_MEMBER} pt
                                                  </span>
                                                ) : (
                                  ...
                                  {ach.warnings && ach.warnings.length > 0 && (
                                    <div className="mt-3 space-y-1 ml-8">
                                      {ach.warnings.map((w, i) => (
                                        <div
                                          key={i}
                                          className="flex items-start gap-1.5 text-yellow-700"
                                        >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <span
                          className={`font-mono text-xs font-bold w-5 shrink-0 ${isMyGroup ? "text-eco-300" : "text-bark/40"}`}
                        >
                          #{index + 1}
                        </span>
                        <div className="min-w-0">
                          <span
                            className={`font-body font-medium text-sm block truncate ${isMyGroup ? "text-cream" : "text-bark"}`}
                          >
                            {g.name}
                          </span>
                          <span
                            className={`font-mono text-xs ${isMyGroup ? "text-eco-300/70" : "text-bark/35"}`}
                          >
                      ...
                      <div className="text-right shrink-0 ml-2">
                        <span
                          className={`font-mono text-xs font-bold block ${isMyGroup ? "text-eco-300" : "text-eco-600"}`}
                        >
                          {g.totalPoints}
                        </span>
                        <span
                          className={`font-mono text-xs ${isMyGroup ? "text-eco-300/60" : "text-bark/30"}`}
                        >
                          credits
                        </span>
                      </div>
